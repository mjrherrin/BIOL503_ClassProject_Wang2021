...
## remove the samples by your threshold (equal or greater to 1185)
nonrare.high <- prune_samples(sample_sums(nonrare) >= 6000, nonrare)

## make another object with only the samples you lost (less than 1185 reads)
nonrare.below6000 <- prune_samples(sample_sums(nonrare) < 6000, nonrare)

## get the metadata out of phyloseq from your low reads object --> 4 samples below cutoff
